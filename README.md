...
В файле `environments.json` хранятся константы, или переменные среды, которые могут упростить хранение огромного количества сложных значений:
...
Константы хранятся в файле, и доступны из любой части. Также их можно создать прям в коде, однако они будут доступны только в 1 модуле:
```js
const name = "супер сложное значение";
...
```
Их можно создавать и вне линий кода:
```js
const count = 16;

PlayerEvent(join) {} // Код линии кода
```
...
import 'module.txt'; // Сохраняем в память код модуля

PlayerEvent(join) {}
use 'module.txt'; // Выгружаем код модуля из памяти

// module.txt

Function() {}
```
Важно помнить, что `import` и `use` доступны только вне кода активаторов. 

При импорте компилятор лишь загружает код файла в память. При этом его код появится только в конце всего кода. Чтобы самому задать место установки кода модуля можно использовать `use`

В модули можно помещать код, и даже создавать константы. Константы, созданные в других модулях могут быть доступны и в других модулях, главное вовремя импортировать и выгрузить код модуля с константой.
