...
При импорте компилятор лишь загружает код файла в память. При этом его код не появится. Чтобы задать место установки кода модуля можно использовать `use`

Чтобы млкт не считали не удобным, необязательно всегда импортировать модуль, можно сразу использовать `use`, он автоматически импортируется. 

Если вы любитель импортов, и не хотите писать для каждого модуля `use`, то можно *импортировать* каждый модуль, а затем использовать `use all;`. Такая конструкция выгружает все не импортированные модули. 
