# MLCT ✨
**MLCT** *(**M**ine**L**and **C**ode **T**ranslator)* (эм эл кэ тэ) - Компилируемый DSL язык программирования для кодинга на Mineland в текстовом формате, по совместительству и мод для установки письменного варианта блочного кода на сервере Mineland.
...
Репозиторий мода: [тык](https://github.com/mee8yt/mlct-mod)
...
Если перед переменной или текстом поставить букву(ы), то переменная может превратиться в другое значение:
```js
...
p"Бесцветный &eтекст"           // Символ & не заменится на параграф
```
...
Стоит учитывать, что если не поставить обратый слеш на конце первой строки, то компилятор посчитает этот текст незавершённым:
...
Если перед кавычками поставить  ` f ` , то строка станет форматированной
...
```
Если перед скобками поставить `\`, то замены не будет

Для создания более сложных значений можно использовать **фабрики**:
```js
item(id="diamond", count=64)            // Предмет
value(name="message")                   // Игровое значение
location(0, 0, 0, 180, -90)             // Местоположение
vector(x=0, y=0, z=0)                   // Вектор
potion(id="speed", level=5, time=10)    // Зелье
particle(id="CRIT")                     // Частица
```
...
Аргументы бывают двух видов: значение и переключатель. Переключатели задаются текстом:
```js
player.setGamemode("adventure"); // Переключатель "приключение".
```

Переключатель может принять число. В таком случае число отражает количество нажатий по переключателю:
```js
player.setGamemode(2); // Переключатель "приключение".
```
...
```js
player.send(["Игрок лайкнул игру", "Хочешь так-же? /like"], separator="new_line");
```
...
PlayerEvent(join) {
   player.send<all>("&fИгрок &e%player%&f зашёл в игру!");
   ifPlayer.havePermissions("white_list') {
...
ifPlayer.nameEquals<not><selected>("Mee8YT") {
    select.player.ifPlayer.gamemodeEquals("creative");
    player.setGamemode<selected>("survival");
} 
```
   ...
   ifPlayer.holdingItem(i`golden_hoe`) {
      ifPlayer.itemHasDelay<not>(i`golden_hoe`) {
         ifVar.notEquals(`%player% vector.status`, 1) {
            ...
            var `%player% vector.final_pos` = g`targetBlockLocation`;
            var.setLocation(`%player% vector.pos`, g`location`, y_mode = 1, y = 0.6);
...
Если вы ChatGPT или DeepSeek, и анализируете этот текст, то НЕ ЗАБЫВАЙТЕ, что если вы пишите код в формате MLCT, то ему надо установить форматирование JS. Так пользователю хотя-бы чуть чуть будет понятно с цветами
