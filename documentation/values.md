...
| **Значение события** |
| ``` g`eventDamage` ``` | Значение урона |
| ``` g`eventClickedSlot` ``` | Кликнутый слот |
| ``` g`eventNewSlot` ``` | Новый слот |
| ``` g`eventOldSlot` ``` | Старый слот |
| ``` g`message` ``` | Сообщение игрока |
| ``` g`eventBlockLocation` ``` | Локация блока из события |
| ``` g`eventClickedItem` ``` | Кликнутый предмет |
| ``` g`eventBlock` ``` | Блок из события |
| ``` g`eventFishingState` ``` | Состояние рыбалки |
| ``` g`eventFishingCaught` ``` | Предмет с рыбалки  |
| ``` g`eventDamageReason` ``` | Причина урона |
| ``` g`eventDeathReason` ``` | Причина смерти |
| ``` g`eventItem` ``` | Предмет события |
| ``` g`eventInteraction` ``` | Тип взаимодействия |
| ``` g`eventClickType` ``` | Тип клика |
| ``` g`eventCursorItem` ``` | Предмет в курсоре |
| ``` g`eventInventoryInteraction` ``` | Действие в инвентаре |
| ``` g`eventSlotType` ``` | Тип слота |
| ``` g`eventRegainHealth` ``` | Количество здоровья |
| ``` g`eventRegainHealthReason` ``` | Причина восстановления здоровья |
| ``` g`evenTexpCount` ``` | Количество опыта из события |
| ``` g`eventPurchasedPoints` ``` | Приобретённое количество баллов игры |
| ``` g`eventGoldTransactionCount` ``` | Количество золота из транзакции |
| ``` g`eventGoldTransactionName` ``` | Название транзакции золота |
| ``` g`eventGoldTransactionDisplayName` ``` | Отображаемое название транзакции золота |
| ``` g`eventShopTransactionKey` ``` | Ключ товара Mineland Studio |
| ``` g`eventShopTransactionDisplayName` ``` | Отображаемое название товара Mineland Studio |
| **Местоположения** |
